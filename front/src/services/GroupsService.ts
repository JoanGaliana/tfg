...
export function useUserGroupsQuery(userId: number | undefined, authToken: string) {
  ...
  return useQuery(
    ['userGroups', userId],
    ({ signal }) => axios.get<getUserGroupsResponse>(`${API_URL}/users/${userId}/groups`, { signal, headers }),
    {
...
export function useCreateGroupMutation({ onSuccess, authToken }: UseCreateGroupMutationParams) {
  ...
  return useMutation(
    (data: CreateNewGroupRequest) => axios.post<CreateNewGroupResponse>(`${API_URL}/groups`, data, { headers }),
    { onSuccess }
  )
}
...
export function useGetGroupByIdQuery(id: string | undefined, authToken: string) {
  ...
  return useQuery(
    ['groups', id],
    ({ signal }) => axios.get<GetGroupByIdResponse>(`${API_URL}/groups/${id}`, { signal, headers }),
    {
      enabled: !!id,
      select: (response) => response.data
    }
